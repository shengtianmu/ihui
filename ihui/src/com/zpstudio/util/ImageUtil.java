...
public class ImageUtil {
	...
	public static void loadImageAsync(final String uri, final int targetWidth,
			final int targetHeight, final LoadingListener listener) {
		final int SUCCESS = 0;
		final int FAILED = 1;
		final Handler myHandler = new Handler() {
			public void handleMessage(Message msg) {
				...
				case SUCCESS:
					if(null != listener)
					{
						listener.onLoadedComplete((byte[])msg.obj);
					}
					break;
				case FAILED:
					if(null != listener)
					{
						listener.onLoadedFail((String)msg.obj);
					}
					break;
				default:
					break;
		...
		new Thread() {
			void sendSuccessMsg(byte[] array){
				Message message = new Message();
				message.what = SUCCESS;
				message.obj  = array;
				myHandler.sendMessage(message);
			}
			void sendFailedMsg(String e){
				Message message = new Message();
				message.what = FAILED;
				message.obj  = e;
				myHandler.sendMessage(message);
			...
			public void run(){
				Bitmap bmp = ImageLoader.getInstance().loadImageSync(uri, new ImageSize(targetWidth, targetHeight));
				if(null != bmp)
				{
					byte[] array = ImageUtil.bmpToByteArray(bmp, true);
					sendSuccessMsg(array);
				}
				else
				{
					sendFailedMsg(uri);
			...
			public void run1() {
				...
				try {
					...
					byte[] array = ImageUtil.bmpToByteArray(thumbBmp, true);
					sendSuccessMsg(array);
					return;
				} catch (MalformedURLException e) {
	...
	public interface LoadingListener {
		void onLoadedComplete(byte[] array);

		void onLoadedFail(String e);
	}
}
